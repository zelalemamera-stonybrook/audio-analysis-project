...
Class Network(nn.Module):
	...
	def __init__(self):
		...
		self.conv2 = nn.Conv1d(1,1, 5, 2) 
	
		self.encode_in = torch.rand((1000, 100), requires_grad = True)
		self.encode_in_bias = torch.rand((1000), requires_grad = True)
		self.encode_hidden = torch.rand((1000,1000), requires_grad = True)
		self.encode_hidden_bias = torch.rand((1000), requires_grad = True)
	
		self.attnlayer1 = torch.rand((2,2000), requires_grad = True)
		...
		self.recurrent_left_in = torch.rand((500,1000), requires_grad = True)
		self.recurrent_left_in_bias = torch.rand((500), requires_grad = True)
		self.recurrent_left_hidden = torch.rand((500,500), requires_grad = True)
		self.recurrent_left_hidden_bias = torch.rand((500), requires_grad = True)
		
		self.recurrent_right_in = torch.rand((500,1000), requires_grad = True)
		self.recurrent_right_in_bias = torch.rand((500), requires_grad = True)
		self.recurrent_right_hidden = torch.rand((500,500), requires_grad = True)
		self.recurrent_right_hidden_bias = torch.rand(500), requires_grad = True)
	...
	def forward(self, input: Tensor):
		'''
		passes the input once through the network, and returns the output. 
		input shape: (n, 10,000)
		output shape: (n, 2)
		where n is the number of syllables >= 2
		'''
		embedded_list = []
		for syll_tensor in input:
			embedded_list.append(self, embed(syll_tensor))
		injected_list = []
		for embedding in embedded_list:
			injected_list.append(inject_features(self, embedding))
		output_list = rnn_forward(self, injected_list)
	...
	def inject_features(self, embedding: Tensor):
		'''
	...
	def rnn_forward(self, injected_list: list[Tensor])
		'''
		passes the list of feature injected and attention weighted tensors through one pass of a bi-directional reccurrent network, and returns the output sequence as a list
		input shape: list[(1000)]
		output shape: list[(2)]
		'''

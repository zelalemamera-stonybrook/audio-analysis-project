...
def train(network: Network , input: list, gold: list, features: list):
	'''
	...
	while epoch > 0:
		...
		for i in n:
			x, y = torch.tensor(input[i]), torch.tensor(gold[i])
			f = torch.tensor(filter(features, i))
...
def read_in_data():
	...
	for word in input:
		input_tensor.append(word)
	for word in gold:
		gold_tensor.append(word)
	for f in features:
		word_list = []
		for word in f:
...
if __name__ == '__main__':
	...
	print('input shape', len(input), len(input[-1]))
	print('gold shape', len(gold), len(gold[-1]))
	print('features shape', len(features), len(features[-1]), len(features[-1][-1]))
